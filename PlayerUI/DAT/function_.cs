...
namespace Jewellery_System.DAT
{
    class function_
    {
         public double  totalamount { get; set; }
        string i =BUS1.Config.ConnectionString;// @"server=127.0.0.1;user id=root;database=hardwear;default command timeout=1000";//arifpos
      ...
      //  string i = @"database='hardwear'; datasource='192.168.8.15'; username='root'; password='12345';default command timeout=1000";
        ...
        public bool ShowMessage(string message, string Con)
        {
            if (Con != "Confirm")
            {
                if (Con == "Warning")
                {
                    MessageBox.Show(message,BUS1. Config.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else if (Con == "Information")
                {
                    MessageBox.Show(message,BUS1. Config.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                }
                else if (Con == "Error")
                {
                    MessageBox.Show(message, BUS1.Config.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                }
            }
            else if (MessageBox.Show(message, BUS1.Config.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                return true;
            }
            return false;
        ...
        public void getData(AutoCompleteStringCollection dataCollection, string sql)
        {
            ...
            MySqlConnection connection = new MySqlConnection(BUS1.Config.ConnectionString);
